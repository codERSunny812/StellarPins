...
const initilizePassport =()=>{
    ...
    passport.use(new LocalStrategy({ usernameField: 'email' },async(email , password ,done)=>{
        ...
        try {
            ...
            if (!searchedUser) return done(null, false,{message:"incorrect email"});

            // if user found check for the password 
            const validateUser = await bcrypt.compare(password, searchedUser.password);

            if (!validateUser) return done(null, false , {message:"incorrect password"});
